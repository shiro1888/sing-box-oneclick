...
### 可视化看板页
装完会多打印一个**看板页地址** `http://<IP>/panel-xxxx.html`，浏览器打开就是一个自包含页面：两种订阅 URL（一键复制）+ **一键导入按钮**（Clash/Mihomo 走 `clash://install-config`、Shadowrocket 走 `shadowrocket://add/sub://`，点一下直接拉进客户端）+ 二维码（手机扫码导入）+ 节点列表。`install.sh links` 也会打印这些深链文本。随时 `install.sh panel` 重新拿地址。
