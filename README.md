...
sudo bash install.sh panel-pass <密码>   # 给看板页加登录(自定义登录页 + nginx 真鉴权; 关闭: panel-pass off)
...
```bash
sudo bash install.sh panel-pass 'your-strong-pass'   # 打开看板会先跳到登录页(密码限 字母数字 . _ ~ -, ≥6 位)
